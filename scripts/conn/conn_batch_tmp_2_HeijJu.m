...
% Project
PROJECT_DIR     = '/tmp/test_conn2'; % Must match batch_conn_01
PROJECT_FILE    = 'conn_project.mat';           % Project filename
BIDS_DIR        = '/data/mrivault/_2_BACKUP/REPOSITORY/069_BW01/rawdata/';      % BIDS dataset root (for reference)

% fMRIprep data
FMRIPREP_DIR    = '/data/local/069_BW01/fmriprep'; % Root fMRIprep directory (derivatives/fmriprep)
...
% Confound regressors (from fMRIprep)
USE_CONFOUNDS   = 1;                           % 1: Use confounds from desc-confounds_timeseries.tsv
CONFOUND_TYPES  = {'WhiteMatter', 'CSF', 'Framewise Displacement'};  % Confounds to include
...
for s = 1:length(subject_dirs)
    ...
    % ===== CONFOUND REGRESSORS (from fMRIprep) =====
    confounds_found = {};
    if USE_CONFOUNDS && ~isempty(session_funcs)
        % Look for confound timeseries in same location as BOLD
        for f = 1:length(session_funcs)
            % Replace _bold.nii.gz with _desc-confounds_timeseries.tsv
            bold_path = session_funcs{f};
            confound_path = strrep(bold_path, '_desc-preproc_bold.nii.gz', '_desc-confounds_timeseries.tsv');
            confound_path = strrep(confound_path, '.nii.gz', '_confounds.tsv'); % Alternative naming
            
            if isfile(confound_path)
                confounds_found{end+1} = confound_path;
            elseif isfile(strrep(bold_path, '_desc-preproc_bold.nii.gz', '_desc-confounds_timeseries.tsv'))
                confounds_found{end+1} = strrep(bold_path, '_desc-preproc_bold.nii.gz', '_desc-confounds_timeseries.tsv');
            end
        end
    end
